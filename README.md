...
**Tailwind v3.4 + custom tokens (not v4).** Tailwind v4 broke shadcn templates we'd planned to use; v3.4 is the stable target. Our `globals.css` is the source of truth for the entire token system (CSS variables, not Tailwind config). Switching dark mode is a matter of swapping variable values.
